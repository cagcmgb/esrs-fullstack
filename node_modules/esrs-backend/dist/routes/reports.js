...
async function assertCanViewReport(user, reportType) {
...
reportsRouter.get('/operating-mines', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    await assertCanViewReport(req.user, ReportType.OPERATING_MINES);
    const { regionCode, commodityId, mineralType } = req.query;
...
reportsRouter.get('/directory', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    ...
    for (const c of contractors) {
        ...
        const approvedDates = activePermits.map((p) => p.dateApproved).filter(Boolean);
        const expiryDates = activePermits.map((p) => p.dateExpiration).filter(Boolean);
...
reportsRouter.get('/production', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    await assertCanViewReport(req.user, ReportType.PRODUCTION);
    const { year, month, quarter, regionCode, commodityId, mineralType } = req.query;
...
reportsRouter.get('/sales', asyncHandler(async (req, res) => {
    ...
    for (const s of filtered) {
        const sales = s.sales ?? {};
        const records = Array.isArray(sales.records) ? sales.records : [];
        if (records.length === 0) {
        ...
        for (const r of records) {
            ...
            const values = [
                ...
                Number(r.fobValuePhp ?? 0),
                Number(r.fobValueUsd ?? 0),
                r.isExport ? 'YES' : 'NO'
            ];
            values.forEach((v, i) => {
                row.getCell(i + 1).value = v;
...
reportsRouter.get('/export-by-country', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    ...
    for (const s of filtered) {
        ...
        for (const r of records) {
            ...
            agg[key].php += Number(r.fobValuePhp ?? 0);
            agg[key].usd += Number(r.fobValueUsd ?? 0);
        }
    }
...
reportsRouter.get('/employment', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    ...
    for (const s of filtered) {
        ...
        const totalMale = Number(emp.totalMale ?? 0);
        const totalFemale = Number(emp.totalFemale ?? 0);
