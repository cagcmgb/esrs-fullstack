...
reportPermissionsRouter.delete(
  '/:role/:reportType',
  asyncHandler(async (req, res) => {
    const { role, reportType } = req.params;
    await prisma.reportPermission.deleteMany({ where: { role: role as any, reportType: reportType as any } });
    res.json({ ok: true });
  })
);
