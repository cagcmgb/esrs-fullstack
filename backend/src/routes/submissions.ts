...
submissionsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { contractorId, year, month, status } = req.query as any;
...
submissionsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    if (!req.user) throw unauthorized();
    const { year, month, quarter, regionCode, commodityId, mineralType, page = '1', limit = '10' } = req.query as any;
...
submissionsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!contractor.isVerified) throw badRequest('Contractor must be verified before encoding data');
