...
exchangeRatesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { year, month, currencyPair = 'USD/PHP' } = req.query as any;
...
exchangeRatesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { currencyPair } = req.query as any;
    const where: any = {};
    if (currencyPair) where.currencyPair = String(currencyPair);
...
exchangeRatesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!year || !month || rate == null) throw badRequest('year, month, and rate are required');
...
exchangeRatesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!effectiveFrom || rate == null) throw badRequest('effectiveFrom and rate are required');
...
exchangeRatesRouter.post(
  '/override',
  requireRole([UserRole.ADMIN, UserRole.CENTRAL_OFFICE, UserRole.REGIONAL_ECONOMIST]),
  asyncHandler(async (req, res) => {
    if (!req.user) throw unauthorized();
...
exchangeRatesRouter.post(
  '/excise-tax/override',
  requireRole([UserRole.ADMIN, UserRole.CENTRAL_OFFICE]),
  asyncHandler(async (req, res) => {
    if (!req.user) throw unauthorized();
