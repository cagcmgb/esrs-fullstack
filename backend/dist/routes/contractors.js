...
contractorsRouter.get('/', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    const { verified, q, regionCode, commodityId, mineralType } = req.query;
    const where = {};
    if (verified === 'true')
    ...
    if (effectiveRegion)
        where.regionCode = effectiveRegion;
    if (q) {
        where.OR = [
...
contractorsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.user.role === UserRole.REGIONAL_ECONOMIST) {
        ...
        if (!req.user.regionCode)
            throw forbidden('Regional Economist missing regionCode');
...
contractorsRouter.get('/:id', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
...
contractorsRouter.put('/:id', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
...
function defaultIdPrefix(regionName) {
    ...
    if (/NCR/i.test(regionName))
        return 'NCR';
    if (/CAR/i.test(regionName))
        return 'CAR';
    if (/BARMM/i.test(regionName))
        return 'BARMM';
    // fallback: use region code prefix style
    return regionName.replace(/\s+/g, '').slice(0, 6).toUpperCase();
}
contractorsRouter.post('/:id/verify', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    if (![UserRole.ADMIN, UserRole.CENTRAL_OFFICE].includes(req.user.role)) {
...
contractorsRouter.post('/:id/unverify', asyncHandler(async (req, res) => {
    if (!req.user)
        throw unauthorized();
    if (![UserRole.ADMIN, UserRole.CENTRAL_OFFICE].includes(req.user.role)) {
